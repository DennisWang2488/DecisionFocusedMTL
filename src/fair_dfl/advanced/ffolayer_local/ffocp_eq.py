"""Local differentiable FFO optimization layer support for the ffo method."""
...
def _make_ffo_fn(mt: "_FFOLayer", solver_args: dict):
    ...
    class _FFOLayerFn(torch.autograd.Function):
        ...
        def backward(ctx, *dvars):
            ...
            # TorchExpression (cvxtorch) stores constants on CPU at build time.
            # To avoid device mismatch when ctx.device is CUDA, compute the
            # Lagrangian terms on CPU and move gradients back afterwards.
            comp_device = torch.device('cpu')

            new_sol = [to_torch(v, ctx.dtype, comp_device) for v in new_sol_lagrangian]
            vars_old = [to_torch(ctx.sol_numpy[j], ctx.dtype, comp_device) for j in range(num_vars)]

            new_eq_dual_t = [to_torch(v, ctx.dtype, comp_device) for v in new_eq_dual]
            old_eq_dual_t = [to_torch(v, ctx.dtype, comp_device) for v in ctx.eq_dual]

            old_scalar_dual_t = [to_torch(v, ctx.dtype, comp_device) for v in ctx.scalar_ineq_dual]
            new_active_dual_t = [to_torch(v, ctx.dtype, comp_device) for v in new_active_dual]

            new_exp_dual_t = [
                [to_torch(new_exp_dual[j][k], ctx.dtype, comp_device) for k in range(3)]
                for j in range(len(ref["exp_cones"]))
            ]
            new_psd_dual_t = [to_torch(v, ctx.dtype, comp_device) for v in new_psd_dual]

            new_pnorm_lam_t = [to_torch(v, ctx.dtype, comp_device) for v in new_pnorm_lam]

            params_req = []
            req_grad_mask = []
            for p in ctx.params:
                need = bool(getattr(p, "requires_grad", False))
                q = p.detach().to(comp_device)
                if need:
            ...
            for need in req_grad_mask:
                g = next(it) if need else None
                if g is not None and comp_device != ctx.device:
                    g = g.to(ctx.device)
                grads.append(g)
